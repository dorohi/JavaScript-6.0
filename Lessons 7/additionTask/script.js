...
fly.addEventListener('click', () => {
	console.log('Click!');
	let toDown = field.clientHeight - fly.clientHeight,
		toRight = field.clientWidth + fly.clientWidth;
	animate({
		duration: 5000,
		timing: makeEaseInOut(bounce),
		draw: function (progress) {
			fly.style.top = toDown * progress + 'px';
		}
	});
	animate({
		duration: 5000,
		timing: circ,
		draw: function (progress) {
			fly.style.left = toRight * progress + 'px';
		}
	});
	animate({
		duration: 5000,
